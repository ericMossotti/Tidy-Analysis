...
Of the other columns, it seems that the start_time, end_time, start_station, and end_station could show if there are hidden duplicated observations. There is no mention that more than one bike can be checked out at once. One big limitation of this analysis is that it's based on a simulated dataset. I wanted to reach a broad audience and I know Google Data Analytics course community would be able to relate, at the very least. I have other more specific interests so I'm not limiting myself to fictional datasets by any means, in my mind. I used the course materials as inspiration, mainly for organizational purposes.

For the sake of this exercise I have assumed that due the logistics of having the exact same times/dates and stations for two different riders is impossible. The other option would be to assume one person could check out multiple cycles. It's only happened 18 times over the course of a full year. I would think that would happen more often because people probably ride in groups often. Scenarios where one person pays and gets Venmo'd later by their friends. In the real world, we would want to be absolutely sure, but for this exercise, it is reasonable to assume these are errors.
...
# Data Browsing: Lighter Lifting with Apache Arrow
...
sql_ridetypeCounts <- paste0(
    "SELECT rideable_type, count() AS 'Count Per Ride-type' ",
    "FROM duckbase ",
...
newPlot <- ggplot2::ggplot(data = fres_rideTypeCounts,
                           mapping = ggplot2::aes(
                               rideable_type,
                               y = 'Count Per Ride-type' * 10^-6))

newPlot2 <- newPlot + ggplot2::geom_col()
