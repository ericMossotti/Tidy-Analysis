...
count_ridetypes <- dplyr::tbl(dbconn, 
                            "trip_data") |>
    dplyr::select(rideable_type) |>
    dplyr::group_by(rideable_type) |>
    dplyr::summarize(type_count = count(), 
                     .groups = "keep") |>
    dplyr::arrange(type_count) |>
    arrow::to_arrow() |>
    dplyr::collect()
...
fres_rideTypeCounts |>
    ggplot2::ggplot(data = fres_rideTypeCounts,
                           mapping = ggplot2::aes(
                               x = (reorder(
                                   rideable_type,
                                   rt_counts)), 
...
-   continuous and discrete data
    ...
    -   time series scatter plots
        ...
        -   color points by rider type

        -   color points by bike type

        -   independent variables = month/day of week/hour of day

        -   dependent variables = rider counts

    -   Type 1 Error rate of hypothesis tests

        -   risk of rejecting the null hypothesis if the null hypothesis is true
...
Pretest-Posttest Design with One Group:
