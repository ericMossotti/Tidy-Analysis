...
# if i wanted to display large numbers with commas and months as names
riders_monthly |>
...
p <- ggplot2::ggplot(riders_monthly,
                     ggplot2::aes(x = reorder(month, rider_count),
                                  y = rider_count,
                                  fill = rider_count)) +
...
queryResult <- duckdb::dbSendQuery(conn = dbconn,
                                   statement = bikeType_count)

fetchedRes <- duckdb::dbFetch(queryResult)

fetchedRes

p1 <- ggplot2::ggplot(data = fetchedRes) +
    ggplot2::geom_col(mapping = ggplot2::aes(x = reorder(rideable_type,
                                                         rt_counts),
                                             y = rt_counts,
                                             fill = rt_counts)) +
...
duckdb::dbClearResult(queryResult)
```
