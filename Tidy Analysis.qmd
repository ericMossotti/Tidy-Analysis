...
noting it was useful for me to retain certain rows at first to determine if there were duplicates. later on, i would not necessarily retain them, though.
...
creates monthly counts for userTypes

```{r}

# Summarizing the monthly membership status of riders in a joined table 

dplyr::tbl(dbconn,
           "trip_data") |>
    dplyr::select(month, member_casual) |>
    dplyr::group_by("Month" = month) |>
    dplyr::filter(member_casual == "casual") |>
    dplyr::summarize("Casuals" = count(member_casual)) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "casualUsers_monthly")
...
dplyr::tbl(dbconn,
    ...
    dplyr::summarize("Annual Memberships" = count(member_casual)) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "annualMembers_monthly")

dplyr::tbl(dbconn,
           "casualUsers_monthly ") |>
    dplyr::left_join(y = dplyr::tbl(dbconn,
                                    "annualMembers_monthly")) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "monthly_userTypes")    
...
creates day of week counts for member type

```{r}

dplyr::tbl(dbconn,
           "trip_data") |>
    dplyr::select(weekday, member_casual) |>
    dplyr::group_by(weekday) |>
    dplyr::filter(member_casual == "casual") |>
    dplyr::summarize("Casuals" = count(member_casual)) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "casualUsers_dayofweek")

dplyr::tbl(dbconn, "casualUsers_dayofweek")
...
dplyr::tbl(dbconn,
    ...
    dplyr::group_by(weekday) |>
    dplyr::filter(member_casual == "member") |>
    dplyr::summarize("Annual Members" = count(member_casual)) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "annualMembers_dayOfWeek")

dplyr::tbl(dbconn, "annualMembers_dayOfWeek")

dplyr::tbl(dbconn,
           "casualUsers_dayofweek") |>
    dplyr::left_join(y = dplyr::tbl(dbconn,
                                    "annualMembers_dayOfWeek")) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "DayOfWeek_UserTypes")    

dplyr::tbl(dbconn, "DayOfWeek_UserTypes")
```
...
dplyr::tbl(dbconn,
    ...
    dplyr::group_by(hour) |>
    dplyr::filter(member_casual == "casual") |>
    dplyr::summarize("Casuals_byHour" = count(member_casual)) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "casuals_byHour")

dplyr::tbl(dbconn, "casuals_byHour")
...
dplyr::tbl(dbconn,
    ...
    dplyr::group_by(hour) |>
    dplyr::filter(member_casual == "member") |>
    dplyr::summarize("Members_byHour" = count(member_casual)) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "members_byHour")

dplyr::tbl(dbconn, "members_byHour")
...
dplyr::tbl(dbconn,
    ...
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "userStatus_byHour")    

dplyr::tbl(dbconn, "userStatus_byHour")
```
...
dplyr::tbl(dbconn,
    ...
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "casual_eBikers_monthly")

dplyr::tbl(dbconn, "casual_eBikers_monthly")
...
dplyr::tbl(dbconn,
    ...
    dplyr::filter(member_casual == "member",
                  rideable_type == "electric_bike") |>
    dplyr::summarize("member_eBikers" = count(member_casual)) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "member_eBikers_monthly")

dplyr::tbl(dbconn, "member_eBikers_monthly")


dplyr::tbl(dbconn,
           "casual_eBikers_monthly") |>
    dplyr::left_join(y = dplyr::tbl(dbconn,
                                    "member_eBikers_monthly")) |>
    dplyr::collect() |>
    duckdb::dbWriteTable(conn = dbconn,
                         name = "rider_status_monthly")

dplyr::tbl(dbconn, "eBike_memberStatus_monthly")
```

triptime2distance

start_station_name2member\
start_station_name2type
...
paths[c()] |> purrr::walk(drops_tables)
```
