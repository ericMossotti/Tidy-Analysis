...
```{sql, connection = duckCon, output.var = "Month_Casuals"}
...
```{sql, connection = duckCon, output.var = "AVG_casuals_monthly"}
...
newConnection3 <-
    ...
    dplyr::select(started_at, 
                  ...
                  rideable_type) |> 
    dplyr::group_by(
        MonthNum = lubridate::month(
        lubridate::as_datetime(started_at))) |> 
    dplyr::filter(member_casual == "casual",
                  rideable_type == "electric_bike") |>
    dplyr::summarize(CasualsOnElectric = count(member_casual)) |> 
    dplyr::show_query()
...
```{r}
newConnection3 <- dplyr::tbl(duckCon, "duckbase") |> 
    dplyr::select(started_at, 
                  member_casual, 
                  rideable_type) |> 
    dplyr::group_by(
...
```{sql, connection = duckCon, output.var = "electricCasuals"}
...
```{sql, connection = duckCon, output.var = "electricMembers"}
...
casualsElectric <- tibble::as_tibble(electricCasuals)

casualsElectric
```

```{sql, connection = duckCon, output.var = "v_eCasuals"}

CREATE VIEW v_eCasuals AS 
SELECT MonthNum, COUNT(member_casual) AS CasualsOnElectric
FROM (
  SELECT
    started_at,
...
```{sql, connection = duckCon, output.var = "drop_table"}

DROP TABLE table_var
```
...
```{sql, connection = duckCon, output.var = "whatever"}
SELECT tbl_eCasuals, tbl_eMembers
FROM tbl_eCasuals JOIN tbl_eMembers
```

```{sql, connection = duckCon}

CREATE TABLE all_Electric AS
SELECT tbl_eCasuals, tbl_eMembers
FROM tbl_eCasuals
LEFT JOIN tbl_eMembers
```
