...
```{r}
tibble ( Complete_Observations = length ( oneTibble [[1]] ),
...
dupTibble <- oneTibble |>
    ...
    select ( started_at:end_station_name ) |>
...
uncorrectDistinct <- distinct ( oneTibble, pick ( "ride_id" ) ) |>
...
correctDistinct <- distinct ( oneTibble, pick ( "started_at",
                             "start_station_name",
                             "ended_at", 
                             "end_station_name" ) ) |>
    # count {dplyr}	
...
correctionTibble <- tibble(Uncorrected = uncorrectDistinct[[1]],
...
oneTibble[3] |>
    ...
    separate_wider_delim ( cols = c(start_date),
                           delim = "-",
                           names = c("Start Year", 
                                    "Start Month",
                                    "Start Day"),
