...
 Now there are 15 total columns.
...
The multi-file partitioning of the dataset should result in faster queries. Parquet files require less disc space as well.
...
With DuckDB, approx 0.11s to return query results. Did not increase memory usage on my machine.
...
In memory parquet file query took around 2.55s. Personal observation, increased memory usage by about 1GB. Clear advantage goes to: Duckb.

Running SQL syntax to the DuckDB has the same results as dplyr package syntax. Those are relatively faster and more memory efficient.
