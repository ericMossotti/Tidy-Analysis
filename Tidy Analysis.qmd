...
```{r}
groupCount_rideTypes <- dplyr::tbl(duckCon, "duckbase") |>
...
avg_MonthlyCasuals <- dplyr::tbl(duckCon, "duckbase") |>
...
avg_MonthlyRiders <- dplyr::tbl(duckCon, "duckbase") |>
    dplyr::group_by(MonthNum = lubridate::month(
        lubridate::as_datetime(started_at))) |>
...
rsums_MonthlyRiders <- dplyr::tbl(duckCon, "duckbase") |>
    dplyr::group_by(MonthNum = lubridate::month(
        lubridate::as_datetime(started_at))) |>
    dplyr::summarize(RidesPerMonth = count()) |>
...
```{sql, connection = duckCon, output.var = "sql_avg_month_casual"}
...
```{sql, connection = duckCon, output.var = "dropTable"}

DROP TABLE all_Electric
```

```{r}

listDuckDBinfo <- function(duckCon) {
...
WHERE tbl_eMembers.MonthNum = tbl_eCasuals.MonthNum
```

```{r}

allElectric_tbl <- dplyr::tbl(duckCon, 
                              "tbl_eMembers") |>
