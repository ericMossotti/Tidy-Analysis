...
# Create lists of download / zips / file addresses. 
durls <-
...
pqFilePaths <- sprintf("tripdata/%d-divvy-tripdata.parquet",
                      202301:202312)
...
tibble::tibble("URLs" = durls,
               "Zip File Paths" = zipPaths,
               "File Names" = fileNames,
               "Parquet File Paths" = pqFilePaths)
```
...
I wanted to reduce resource requirements by converting the .CSV files to .Parquet so that there are no unneeded copies of info. The only original copies of data I want to keep in this project will be compressed files in the ZIP folder.

```{r}

# A custom function that makes unzipping, converting, and relocating files all at once easy and fun! 
unz_csv2pq <- function (x = pqFilePaths,
                        y = zipPaths,
                        z = fileNames) {
    for (i in seq(x)) {
        utils::unzip (y[i],
...
# Make a more memory efficient dataset for analysis.
