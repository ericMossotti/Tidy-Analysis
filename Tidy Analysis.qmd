...
# counting the rider status for trip times 
dplyr::tbl(dbconn,
           "trip_data") |>
    dplyr::select(trip_time,
                  member_casual) |>
    dplyr::group_by("RiderStatus" = member_casual) |>
    dplyr::filter(trip_time <= 1) |>
    dplyr::summarize("trip times <= 1 min." = dplyr::count(member_casual),
                     .groups = "keep") |>
    dplyr::arrange()
```
...
dplyr::tbl(dbconn,
    ...
    duckdb::dbWriteTable(conn = dbconn,
                         name = "datetime_MemberCounting")

dplyr::tbl(dbconn,
           "datetime_MemberCounting") |>
    head()
```

## Rider Status and Trip Duration Tables
