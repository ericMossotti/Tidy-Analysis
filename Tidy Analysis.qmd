...
# A custom function that makes unzipping, converting and relocating files all at once easy and fun! 
...
```{r}
cat("Row count of total observations before any cleaning:", countRows[[1]])
```

```{r}

temp_df <- dplyr::tbl(dbconn,
           "trip_data") |>
    as.data.frame()

temp_df <- temp_df |>
    dplyr::mutate(ride_id = dplyr::na_if(ride_id, ""),
...
Run a count on how many rows or observations there are in our dataset.
...
Of the other columns, it seems that the start_time, end_time, start_station, and end_station could show if there are possibly hidden duplicated observations. There is no mention that more than one bike can be checked out at once in the project description.

I assumed that having the exact same times/dates and stations for two different ride IDs is a mistake. Although, I do not know how that error would happen.

I could have assumed one person could check out multiple cycles at once. That, however, has only happened 18 times over the course of a full year. One would suppose that would happen in groups more often than 1-2 times a month, on average. A scenario where one person pays and gets "*Venmo'd*" later by their friends comes to mind.
...
distinct_n <- dplyr::tbl(dbconn,
                         "dupeTable") |>
    dplyr::distinct(n) |>
    as.data.frame()

cat("All distinct values of duplicates, n =", distinct_n[[1]])
```
...
Our predicted value for observations in the undupedTable was indeed 18.
