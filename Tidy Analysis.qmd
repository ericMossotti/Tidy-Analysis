...
# A way to efficiently create a list of download addresses. 
durls <-
...
```{r}
# Verifying data type for thoroughness. 
typeof(oneTibble)
...
# Arrow dataset that points to one .CSV file
onecsvset <- arrow::open_csv_dataset(sources = "oneCSVfile")
```

We test the performance of queries to a one .CSV file dataset.
...
Testing the performance of queries to a "partitioned" .CSV file dataset.
...
Testing the performance of queries to a partitioned .Parquet file dataset.
