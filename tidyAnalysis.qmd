...
dplyr::tbl(dbconn,
           "trip_data") |>
    # Filtering because there were trips with negative trip times.
    # Also, possibly filtering out people who changed their minds about riding, and would therefore have very low trip_times.
    # There could also be a chance of nonsensical, randomly generated data from the simulated dataset.
    dplyr::filter(trip_time > 1,
                  trip_time < 480) |>
    dplyr::collect() |>
    dplyr::mutate(
    ...
    # Might also be nonsensical, randomly generated data.
    dplyr::filter(miles > 0.1,
                  # Seems that pro cyclists average around 20 mph, so I set that as the ceiling.
                  # https://bikingultimate.com/average-bicycle-speed-how-fast-do-cyclists-ride-and-what-affects-their-pace/
                  mph < 21,
                  # To possibly account for time spent idling.
                  # People probably aren't riding around without a track.
                  # To account for stoplights and traffic.
                  mph > 1) |>
...
#    arrow::to_arrow() |>
#    arrow::write_parquet(sink = "fltrd")
...
ggplot2::ggplot(data = hours_of_Riders,
                mapping = ggplot2::aes(x = hour,
                                       y = meanCount,
                                       fill = meanCount)) +
    ggplot2::geom_col() +
    ggplot2::coord_radial(start = 2*pi,
                          inner.radius = .3) +
    ggplot2::scale_y_continuous(name = "Avg_Riders") +
    ggplot2::xlab("Hour of Day (24-hour)") +
    ggplot2::scale_fill_distiller(palette = 'Spectral',
                                  direction = 1) +
    ggplot2::labs(
    title = "Average Rides by Hour of Day",
    subtitle = "(Jan-Dec, 2023)",
    caption = "Data from cyclistic database.",
    tag = "Figure 1.c",
    fill = "") +
...
visualizations by month/hour/min
